...
class TransformerDualHead(nn.Module):
    def __init__(self, encoder: nn.Module, n_clinical: int, n_drug: int = 1,
                 ...
                 modality_dropout: float = 0.1, instance_dropout: float = 0.15,
    ...
    def forward(self, clinical, drug, bag_images, bag_mask, task=None):
        ...
        if self.mode == "both":
            ...
            if self.training and self.modality_dropout > 0:
                drop_mask = (torch.rand(B, 1, 1, device=clinical.device) <= self.modality_dropout).float()
                clin_token = clin_repr.unsqueeze(1) * clin_scale * drop_mask
            else:
                clin_token = clin_repr.unsqueeze(1) * clin_scale
...
def build_image_model(model_name: str, encoder: nn.Module, n_clinical: int, n_drug: int, cfg_stage23: dict):
    ...
    modality_dropout = cfg_stage23.get("modality_dropout", 0.3)
    instance_dropout = cfg_stage23.get("instance_dropout", 0.4)
    use_checkpoint = cfg_stage23.get("use_checkpoint", True)
    clinical_hidden = cfg_stage23.get("clinical_hidden", 16)
    warmup_epochs = cfg_stage23.get("warmup_epochs", 5)
    return TransformerDualHead(
        ...
        dropout=dropout, modality_dropout=modality_dropout, instance_dropout=instance_dropout,
...
def dualhead_loss_with_constraint(outputs, rem_labels, hea_labels, crit,
    ...
    rem_correct = (rem_prob >= 0.5).float()
    rem_wrong = 1 - rem_correct
    rem_confidence = torch.max(rem_prob, 1 - rem_prob)
    focus_weight = rem_wrong * rem_confidence
    focus_loss = (focus_weight * torch.nn.functional.binary_cross_entropy_with_logits(
        rem_logits, rem_labels, reduction='none'
    )).mean()

    total = rem_bce + hea_bce + constraint_weight * constraint_loss + 0.1 * focus_loss
...
def train_and_evaluate_on_test(model_name, train_df, test_df, image_df, cfg, encoder, clinical_cols, n_drug, device, use_amp):
    ...
    optimizer_name = stage_cfg.get("optimizer", "adamw").lower()
    momentum = stage_cfg.get("momentum", 0.9)
    T_max = stage_cfg.get("T_max", 70)
    ...
    encoder_lr = float(stage_cfg.get("encoder_lr", 5e-7))
    clinical_encoder_lr = float(stage_cfg.get("clinical_encoder_lr", 1e-5))
    ...
    for epoch in range(max_epochs):
        ...
        if epoch >= warmup_epochs:
            if rem_auc > best_rem_auc + min_delta:
                acc_drop = best_rem_acc - rem_acc
                if acc_drop > 0.05:
                    print(f"    -> AUC={rem_auc:.4f} ok but acc dropped {acc_drop:.4f}, not counting as best")
                    epochs_no_improve += 1
                    if epochs_no_improve >= patience:
                        print(f"  Early stopping at epoch {epoch+1} (no genuine improvement for {patience} epochs)")
                        break
                else:
                    ...
                    epochs_no_improve = 0
                    print(f"    -> New best! rem_auc={rem_auc:.4f} (acc={rem_acc:.4f}, f1={rem_f1:.4f})")
...
def main():
    ...
    image_size = 336
    unfreeze_blocks = cfg.get("stage3", {}).get("unfreeze_encoder_blocks", 0)
    ...
    for model_name in image_models:
        ...
        if model_name == "both":
            warmup_training_mode = "image_only"
        else:
            warmup_training_mode = model_name
