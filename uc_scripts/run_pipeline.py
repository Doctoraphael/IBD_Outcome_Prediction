...
def build_base_config(root: str) -> dict:
    ...
    return {
        "root": root,
        "image_dir": str(Path(root) / "imageUC"),
        "img_csv": str(Path(root) / "uc_image_analysis.csv"),
        "pat_csv": str(Path(root) / "uc_patient_summary.csv"),
        "gastronet_ckpt": str(Path(root) / "hf_cache" / "dinov2.pth"),
        "gastronet_model_name": "vit_base_patch14_reg4_dinov2",
        "output_dir": str(Path(root) / "outputs_uc_gastronet5m"),
        ...
        "stage3": {
            ...
            "warmup_epochs": 5,
            "warmup_encoder_lr": 5e-7,
            "warmup_head_lr": 1e-6,
            "lr": 5e-05,
            "encoder_lr": 1e-05,
            "clinical_encoder_lr": 1e-05,
            "clinical_low_lr_epochs": 5,
            "optimizer": "adamw",
            "momentum": 0.9,
            "T_max": 70,
            "weight_decay": 0.15,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run 4-stage UC pipeline with GastroNet-5M backbone for biological agent outcome prediction"
    )
    parser.add_argument(
        "--root", 
        default=r"C:\crohn",
        help="项目根目录，包含imageUC文件夹和CSV文件"
    )
    parser.add_argument(
        "--output-dir", 
        default=None,
        help="输出目录路径，默认为{root}/outputs_uc_gastronet5m"
    )
    parser.add_argument(
        "--config-out", 
        default=None,
...
def main():
    ...
    config_path = Path(args.config_out) if args.config_out else out_dir / "uc_pipeline_config.json"
    ...
    if not split_info_path.exists():
        ...
        pat_df["drug_enc"] = (pat_df["drug"].astype(str) == "VDZ").astype(float)
